...
# Setup Markdown output
md_lines = []
md_lines.append("# AI 應用初級課程題庫解析")
md_lines.append("\n本檔提供題庫題目的完整解析與說明。\n")
md_lines.append("---\n")
...
for idx, (q_text, options) in enumerate(clean_qa, 1):
    ...
    for o_idx, o_txt in enumerate(options):
        ...
        if o_idx == ans_idx:
            ...
            md_lines.append(f"- <span style='color: #0066cc; font-weight: bold;'>({letter}) {o_txt}</span>")
        else:
            md_lines.append(f"- ({letter}) {o_txt}")
    
    md_lines.append("")
    md_lines.append(f"<span style='color: #008000; font-weight: bold;'>✅ 正確解答：({chr(65+ans_idx)}) {ans_text}</span>  ")
    md_lines.append(f"<span style='color: #646464;'>💡 為什麼？ {ans_exp}</span>")
    md_lines.append("\n---\n")

save_path = r'c:\雲端\ipas AI應用證照\題庫\AI應用初級題庫_解答與解析.md'
with open(save_path, 'w', encoding='utf-8') as f:
    f.write('\n'.join(md_lines))

print(f"Saved formatted QA markdown to {save_path}")
